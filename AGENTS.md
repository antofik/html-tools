...
- **Diff Viewer** (`diff_viewer.html`)
  ...
  - Supports `Plain text`, `Env`, `Log`, `CSV`, `JSON`, and `YAML` comparison modes.
  - `Env` mode compares `key=value` entries by key and only performs inline comparison for matching keys.
  - `CSV` mode parses the first row as headers and can compare only selected columns.
  ...
  - Can ignore row order by sorting comparable rows before diffing.
